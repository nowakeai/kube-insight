...
import type { AgentArtifact, AgentArtifactKind, AgentCitation, AgentPanelWorkspace, AgentRun, AgentRunEvent, AgentRunStatus, AgentSession } from "@/lib/agent-store-types"
export type { AgentArtifact, AgentArtifactKind, AgentCitation, AgentPanelWorkspace, AgentRun, AgentRunEvent, AgentRunStatus, AgentSession } from "@/lib/agent-store-types"
      ...
      for (const session of sessions) {
        for (const run of serverSessionRuns(session)) {
          ...
          runs[run.id] = {
            ...
            finalAnswer: run.finalAnswer ?? previousRun?.finalAnswer,
  ...
  upsertServerRun: (run, options = {}) => {
    ...
    set((current) => {
      ...
      const nextRun: AgentRun = {
        ...
        finalAnswer: run.finalAnswer ?? previous?.finalAnswer,
