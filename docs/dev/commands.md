...
| Scripted SQL | dependent profile -> proof SQL, several independent aggregates, or SQL rows plus grouping | schema + one `kube_insight_scripted_query` | bounded read-only SQL only; prefer over repeated SQL + transform |
