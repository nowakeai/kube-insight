...
- Use kube_insight_sql for precise backend-specific queries, ranking, aggregation, or proof rows after reading kube_insight_schema. Keep maxRows bounded and select only columns needed for the claim. Skip SQL when typed tools already returned enough evidence for the user's question. For broad OOM/restart summaries, use health plus one targeted Pod search in the first tool group, then schema plus one aggregate facts SQL query if ranking/counts are needed. Do not call history before the aggregate SQL, do not run repeated includeBundles searches for broad summaries, and do not run an exact-name search after a candidate search unless the aggregate is inconclusive. When the aggregate confirms counts/timestamps, answer immediately. Prefer one aggregate over facts filtered to Pod OOMKilled/restart facts and ordered by last_seen/count; avoid observations.doc ILIKE scans unless facts are insufficient or raw document proof is explicitly needed.
...
- Include namespace, kind, cluster, and time predicates when available. For relative-time questions, always include a time predicate on the relevant timestamp column: facts.ts, changes.ts, observations.observed_at, versions.observed_at, edges.valid_from, or ingestion_offsets.updated_at.
- Match the backend's indexed/sorted columns first. For ClickHouse facts prefer cluster_id + exact fact_key/fact_value + ts before text matching; for OOMKilled, use fact_value = 'OOMKilled' rather than LIKE or text contains. Changes prefer cluster_id + change_family/path + ts; observations and versions prefer cluster_id + kind + namespace + name + observed_at before opening doc content.
- Use LIMIT/maxRows. Prefer aggregation for broad questions and proof-row queries for specific claims. Do not run multiple equivalent probes after a zero-result broad search; switch to schema-guided facts/changes/edges aggregation with exact keys, kinds, namespaces, or time windows.
