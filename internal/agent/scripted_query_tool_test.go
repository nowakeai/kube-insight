...
func TestScriptedQueryToolInfoEncouragesQueryPlanning(t *testing.T) {
	...
	for _, want := range []string{scriptedQueryToolName, "sql(query, maxRows)", "sqlAll", "after kube_insight_schema", "dependent or parallel", "Do not use await", "Never write const sql", "history: history", "window_utc", "client_timezone", "do not treat one JS call as a correctness requirement", "Omit maxQueries", "maxQueries is hard-capped at 10", "one SQL query per bucket", "totalCountHistory", "Usually omit maxOutputBytes", "scratch.load(path)", "do not use comma-expression destructuring", "never call this tool only to reformat", "10000", "50000", "Pod-count peak", "pod_count_peak_intervals_for_js", "interval_start, interval_end, and from_baseline", "Do not split that recipe", "negative", "raw Pod observation export returns exactly maxRows", "PVC resize", "JSONExtractString", "parse units in JavaScript", "Date arithmetic", "current_node_capacity", "node_lifecycle_events", "node_net_change", "EndpointSlice", "arrayJoin", "empty endpoints arrays", "ADDED/MODIFIED/DELETED", "pure ADDED/DELETED lifecycle sweep", "not JSON.stringify", "{{evidence: Node capacity snapshot}}"} {
