export function demoAgentAnswer(question: string) {
  return [
    ...
    "",
    "This browser could not reach the kube-insight agent API from the current origin, so the UI is showing the local demo fallback. When the Web UI is served with the Go API, this same composer creates a server session, starts a run, and replays SSE events.",
    "",
    "### Demo findings",
    "| Signal | Status | Evidence |",
    "| --- | --- | --- |",
    "| Agent loop | API fallback | Local demo adapter |",
    "| Markdown renderer | Ready | GFM table and fenced code block |",
    "",
    "```yaml",
    "kind: DemoInvestigation",
    `query: ${JSON.stringify(question)}`,
    "status: fallback-rendered",
    "```",
    "",
