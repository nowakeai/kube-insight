...
func TestDefaultAgentInstructionRequiresEvidenceCitations(t *testing.T) {
	...
	for _, want := range []string{"kube-insight Kubernetes investigation agent", "Evidence first", "kube_insight_health", "kube_insight_schema", "ClickHouse-compatible", "kube_insight_search", "includeDocs false", "maxRows bounded", "imaginary tables such as objects", "failed tool call is diagnostic evidence", "Parallel tool calls", "Visible progress notes", "user-visible progress notes", "private chain-of-thought", "client time zone", "IANA time zone", "cluster display map", "cluster id in parentheses", "isError true", "cite the exact proof", "human-readable evidence label", "{{evidence: OOM facts}}", "query OOMKilled with kind Pod", "Do not call history before the aggregate SQL", "do not run repeated includeBundles searches", "health plus schema", "do not add a cluster_id predicate", "not RFC3339 strings with T or Z", "cluster or UID is known", "one changes SQL query", "exact recent-change", "answer only the observed changes", "rollup-style changes SQL", "group by change_family/path/severity", "do not run spec-only", "Do not use search for exact recent-change", "Do not use topology for a pure recent-change", "Do not run wildcard", "Use evidence_condenser only", "source artifact IDs", "do not pass only your own prose summary", "answer immediately", "relative time words from the client context", "facts.ts, changes.ts, observations.observed_at", "fact_value = 'OOMKilled'", "Avoid ILIKE, LIKE, positionCaseInsensitive", "avoid observations.doc ILIKE scans", "version IDs", "Evidence section"} {
