...
func tools() []sdkmcp.Tool {
	return []sdkmcp.Tool{
		...
		},
		{
			...
			Description: "Summarize collector coverage, staleness, and resource stream health before making current-state claims. Call without a cluster filter at the start of a scoped investigation when the user gives a cluster display/context name or fragment, so the returned clusters map shows available display names and stable cluster_ids. Do not pass a user fragment such as gcp2 as the first health cluster argument; use the stable cluster_id after reading the unfiltered clusters map. The optional cluster argument accepts the stable cluster_id or a known display/context/source alias after discovery. Returns a compact DSL summary plus a bounded list of problematic resources by default; the HTTP /api/v1/health endpoint is much larger and intended for dashboards.",
			InputSchema: map[string]any{
				"type": "object",
				"properties": map[string]any{
