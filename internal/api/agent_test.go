...
func TestCreateAgentRunStartsRunnerAndFollowEvents(t *testing.T) {
	...
	case input := <-runner.inputs:
		...
		for _, want := range []string{"Client context for this run", "UTC for tool arguments", "render final-answer timestamps in the client time zone", "IANA time zone", "Client locale and languages are UI formatting context only", "even when they are values such as zh-CN", "language of the user's current prompt", "Detected current prompt language: English", "Response language for this run: English", "2026-05-24T23:30:00+08:00", "Asia/Shanghai", "zh-CN"} {
