...
func (t ScriptedQueryTool) Info(context.Context) (*schema.ToolInfo, error) {
	return &schema.ToolInfo{
		...
		ParamsOneOf: schema.NewParamsOneOfByParams(map[string]*schema.ParameterInfo{
			...
			"maxQueries": {
				Type: schema.Integer,
				Desc: "Optional maximum SQL calls allowed from the script. Usually omit this and use the default 6. If set, it must be at least the number of sql() calls plus sqlAll() specs the script will run and is hard-capped at 10; rewrite scripts that would need more queries instead of setting maxQueries to 30.",
			},
			"defaultMaxRows": {
				Type: schema.Integer,
...
func (s *scriptedQueryState) reserveQueries(count int) error {
	...
	if s.queries+count > s.maxQueries {
		return fmt.Errorf("script query limit exceeded: %d requested > %d allowed; maxQueries is capped at %d, so rewrite scripts needing more queries as fewer aggregate SQL calls", s.queries+count, s.maxQueries, maxScriptedQueryMaxQueries)
	}
	s.queries += count
	return nil
}
